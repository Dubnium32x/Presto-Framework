...
def compute_widthmap(tile_img):
    """Compute width values for each column in a 16x16 tile."""
    w, h = tile_img.size
    assert w == 16 and h == 16
    pixels = tile_img.load()
    widths = []
...
def process_tileset_height(path):
...
def process_tileset_width(path):
    """Process a tileset image and extract widthmaps for all tiles."""
    print(f'Processing {path}')
    img = Image.open(path).convert('RGBA')
    tw, th = img.size
    tiles_y = tw // 16
    ...
    for tx in range(tiles_x):
        for ty in range(tiles_y):
            box = (ty*16, tx*16, ty*16+16, tx*16+16)
            tile = img.crop(box)
            widths = compute_widthmap(tile)
            maps.append(widths)
    
    print(f'Extracted {len(maps)} tiles from {os.path.basename(path)}')
    return maps

def write_c_files(header_path, source_path, tileset_name, heightmaps, widthmaps):
    """Write heightmap and widthmap data as C header and source files."""
    
    print("working on heightmaps...")
    # Write header file
    with open(header_path+"HEIGHT.h", 'w') as f:
    ...
    # Write source file  
    with open(source_path+"HEIGHT.c", 'w') as f:
        f.write('// Auto-generated heightmaps file\n')
        f.write('// Generated by tools/generate_heightmaps.py\n\n')
        f.write('#include "'+header_path+'HEIGHT.h"'+'\n\n')
    ...
    print("working on widthmaps...")
    """WIDTHMAP"""
        # Write header file
    with open(header_path+"WIDTH.h", 'w') as f:
        f.write('// Auto-generated widthmaps file\n')
        f.write('// Generated by tools/generate_heightmaps.py\n\n')
        f.write('#ifndef GENERATED_WIDTHMAPS_H\n')
        f.write('#define GENERATED_WIDTHMAPS_H\n\n')
        f.write('#include <stdint.h>\n\n')

        f.write(f'#define TILESET_TILE_COUNT {len(widthmaps)}\n')
        f.write('#define TILE_HEIGHT 16\n\n')

        f.write('// Single tileset widthmaps: [tile][column]\n')
        f.write('extern const int TILESET_WIDTHMAPS[][TILE_HEIGHT];\n\n')
        
        f.write('// Tileset name\n')
        f.write('extern const char* TILESET_NAME;\n\n')
        
        f.write('#endif // GENERATED_WIDTHMAPS_H\n')

    # Write source file  
    with open(source_path+"WIDTH.c", 'w') as f:
        f.write('// Auto-generated widthmaps file\n')
        f.write('// Generated by tools/generate_heightmaps.py\n\n')
        f.write('#include "'+header_path+'WIDTH.h"'+'\n\n')

        f.write(f'const char* TILESET_NAME = "{tileset_name}";\n\n')

        f.write('const int TILESET_WIDTHMAPS[][TILE_HEIGHT] = {\n')
        for i, widthmap in enumerate(widthmaps):
            f.write('    {' + ', '.join(f'{h:2d}' for h in widthmap) + '},')
            if i < 10 or i % 20 == 0:  # Add comments for first few and every 20th tile
                f.write(f' // Tile {i}')
            f.write('\n')
        f.write('};\n')
...
def main():
    ...
    heightmaps = process_tileset_height(tileset_path)
    widthmaps = process_tileset_width(tileset_path)

    # Write output files
    header_path = out_basename
    source_path = out_basename
    
    write_c_files(header_path, source_path, tileset_name, heightmaps, widthmaps)
    
    print(f"Generated {header_path} and {source_path}")
    print(f"Processed HEIGHT : {len(heightmaps)} WIDTH : {len(heightmaps)} tiles from tileset '{tileset_name}'")
