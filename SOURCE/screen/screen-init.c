...
void InitScreen_Init(void) {
    // Load resources
    segaLogo = LoadTexture("RESOURCES/image/logos/Sega-logo.png");
    segaJingle = LoadSound("RESOURCES/sound/sfx/jingle.ogg");
    
    // Check if sound loaded properly
    if (segaJingle.frameCount == 0) {
        printf("Warning: Failed to load Sega jingle sound properly\n");
    } else {
        printf("Sega jingle loaded successfully: %u frames\n", segaJingle.frameCount);
    }
...
void InitScreen_Update(float deltaTime) {
    ...
    if (*initState == INIT_SPLASH) {
        ...
        if (*currentPhase == LOGO_ANIMATE_IN) {
            ...
            if (!jinglePlayed && segaJingle.frameCount > 0) {
    ...
    if (*currentPhase == PHASE_DONE) {
        ...
        printf("Init screen finished, transitioning to title screen...\n");
