...
void TitleCardCamera_Update(float deltaTime) {
    ...
    if (processTimer > BACK_FADE_DELAY && backFadeAlpha > 0.0f) {
    ...
    switch (titleCardState) {
        ...
        case TITLE_CARD_STATE_ENTERING:
            ...
            if (totalDistance < 10.0f) { // Elements are close enough to target positions
                ...
                processTimer = 0.0f; // Reset processTimer for display duration
            }
            break;
        case TITLE_CARD_STATE_DISPLAY:
            ...
            if (processTimer >= 3.0f) {
                ...
                processTimer = 0.0f; // Reset processTimer for exit animation
            }
            break;
        case TITLE_CARD_STATE_EXITING:
            ...
            processTimer += deltaTime;
            if (processTimer >= 1.5f) { // Exit animation duration
                titleCardState = TITLE_CARD_STATE_INACTIVE;
                isTitleCardActive = false;
                StartTimer(); // Restart game processTimer after title card
            }
            break;
        default:
            break;
