...
Tile** LoadTileLayer(const char* csvPath, int* outWidth, int* outHeight) {
    ...
    if (strstr(line, "-1") != NULL) hasMinusOne = true;
    const char* p = line;
        int tokens = 0;
        while (*p && *p != '\n' && *p != '\r') {
    ...
    while (fgets(line, sizeof(line), file)) {
        height++;
        if (!hasMinusOne && strstr(line, "-1") != NULL) hasMinusOne = true;
    }
...
void CalculateLevelDimensions(LevelData* level) {
    ...
    if (level->height <= 0) level->height = 32;
}
...
bool IsTileSolidAtLocalPosition(int tileId, float worldX, float worldY, int tileX, int tileY, 
    ...
    (void)worldY; (void)tileY; // Unused parameters
...
LevelData LoadCompleteLevelWithFormat(const char* levelPath, bool useJSON) {
    ...
    if (tmx) {
        ...
        if (mapW > 0 && mapH > 0) { level.width = mapW; level.height = mapH; }
                ...
                if (enc) {
                    ...
                    if (csvStart && csvEnd && csvEnd > csvStart) {
                        ...
                        if (tmp) {
                            ...
                            if (w>0 && h>0) { level.width = w; level.height = h; }
                            remove(tmpPath);
                        } else {
                            ...
                            // First pass compute width/height
                            {
                                ...
                                while (*p) {
                                    ...
                                    while (*p && *p != '\n' && *p != '\r') {
                                        ...
                                        if (!comma || (nl && nl < comma)) { p = nl ? nl : p + strlen(p); break; }
                                        p = comma + 1;
                                    }
                                    if (rowW > 0) { h++; if (rowW > w) w = rowW; }
                                    while (*p == '\r' || *p == '\n') p++;
                                }
                            }
                            if (w>0 && h>0) {
                                ...
                                while (*p && y<h) {
                                    ...
                                    while (*p && *p != '\n' && *p != '\r' && x<w) {
                                        ...
                                        int tileId = (int)val;
                                        if (tileId <= 0) tileId = 0; else tileId = tileId - firstgid + 1;
