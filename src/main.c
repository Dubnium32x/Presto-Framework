...
int main(void)
{
    ...
    RegisterScreen(&screenManager, SCREEN_LEVEL_EDITOR, LevelEditor_Init, LevelEditor_Update, LevelEditor_Draw, LevelEditor_Unload);
    ...
    // Unload fonts
    if (s1TitleFont.texture.id != 0)
        UnloadFont(s1TitleFont);
    if (s1ClassicOpenCFont.texture.id != 0)
        UnloadFont(s1ClassicOpenCFont);
    if (sonicGameworldFont.texture.id != 0)
        UnloadFont(sonicGameworldFont);
